...
<body class="">
  ...
  <div class="main-content">
   ...
   require_once('menu/menutop.php');
    $pedidos = $con->query("SELECT p.*, dc.* from pedidos p INNER JOIN datos_clientes dc where p.id_datos_cliente=dc.id AND p.numero_pedido='$numero_pedido' ORDER BY p.id DESC");
     ?>
    <!-- Header -->
        ...
        <div class="col">
          <div class="card shadow">
            <div class="row">
              ...
              <h3 class="mb-0">Detalles del pedido #<?php echo strtoupper($numero_pedido); ?></h3>
            ...
            <div class="row">
              <div class="col-md-6">
                ...
                <table class="table-fill">
                    ...
                    <tr>
                    <td class="text-left"><strong>Número de Cliente:</strong></td>
                    <td class="text-left"> <?php echo "000".$rowpedidos->id_datos_cliente; ?></td>
                    </tr>
                    <tr>
                    <td class="text-left"><b>Nombre del cliente:</b></td>
                    <td class="text-left"><?php echo $rowpedidos->nombreCompleto ?> <a href="clientes_detalles.php?id_cliente=<?php echo $rowpedidos->id_datos_cliente; ?>"><i class="fas fa-plus-circle"></i></td>
                    </tr>
                    <tr>
                    <td class="text-left"><b>Dirección de entrega:</b></td>
                    <td class="text-left">
                        <?php 
                    ...
                    <tr>
                    <td class="text-left"><b>Requiere factura:</b></td>
                    <td class="text-left">
                         <?php 
                      if ($rowpedidos->id_facturacion!='0') {
                         ?>
                         <span class="badge badge-info" onclick="verFacturacion('<?php echo $rowpedidos->id_facturacion ?>'); return false;">Ver datos de facturación</span>
                         <?php
                       }else{
                        echo "No";
                       } ?>
                    ...
                    <tr>
                    <td class="text-left"><b>Costo y paquetería</b></td>
                    <td class="text-left">
                       $<?php echo $rowpedidos->costoEnvio ?><br>
                      <img src="../assets/images/paqueterias/<?php echo $rowpedidos->paqueteria ?>.png" style="max-height: 50px;" alt="paqueteria">

                    </td>
                    </tr>
                    <tr>
                    <td class="text-left"><b>Monto total del pedido:</b></td>
                    <td class="text-left">
                        $<?php echo number_format((float)$rowpedidos->total, 2, '.', ',') ?> <br>
                    </td>
                    </tr>
                    <tr>
                    <td class="text-left"><b>Metodo de pago:</b></td>
                    <td class="text-left">
                      <badge style="cursor: pointer;" class="badge badge-success"><?php if($rowpedidos->metodoPago=="Stripe"){ echo '<a href="https://dashboard.stripe.com/test/payments?status[0]=successful" target="_blank">'.$rowpedidos->metodoPago.'</a>'; }else{
             ...
             <div class="col-md-6">
                ...
                <table class="table-fill">
                    <tbody class="table-hover">
                    <tr>
                    <td class="text-left"><strong>Estatus del pedido:</strong></td>
                    <td class="text-left">
                    ...
                    <tr>
                    <td class="text-left"><b>Actualizar estatus:</b></td>
                    <td class="text-left">
                         <?php if ($rowpedidos->pagovalido==0) {?>
                         <badge style="cursor: pointer;" class="badge badge-info" onclick="validarPago('<?php echo $rowpedidos->numero_pedido ?>','1'); return false;" >Validar pago</badge><br>
                     <?php }else{ ?>
                       <badge style="cursor: pointer;" class="badge badge-success" onclick="validarPago('<?php echo $rowpedidos->numero_pedido ?>','0'); return false;" ><i class="fas fa-check-circle"></i> Pago validado</badge><br>
                     <?php } ?>
                      <?php if ($rowpedidos->estatus=="proceso") {
                        ?><badge style="cursor: pointer;" class="badge badge-success" onclick="completarPedido('<?php echo $rowpedidos->numero_pedido ?>'); return false;" >Finalizar</badge><br><?php
                      }else{ ?>
                      <badge style="cursor: pointer;" class="badge badge-info" onclick="enviar('<?php echo $rowpedidos->numero_pedido ?>'); return false;" >Enviar pedido</badge><br>
                    <?php } ?>
                      <!-- <badge style="cursor: pointer;" class="badge badge-warning">Editar</badge> -->
                      <badge style="cursor: pointer;" class="badge badge-danger" onclick="cancelarPedido('<?php echo $rowpedidos->numero_pedido ?>'); return false;">Cancelar Pedido</badge>
                    </td>
                    </tr>
                    <tr>
                    <td class="text-left" colspan="2"><b>Productos comprados:</b></td>
                  </tr>
                  <tr>
                       ...
                       $num_pedido = $rowpedidos->numero_pedido;
                          $pedidos_detalles = $con->query("SELECT p.*, pd.* FROM productos p LEFT JOIN pedidos_detalles pd ON p.id=pd.id_producto WHERE numero_pedido='$num_pedido'");
                           while($rowpedidos_detalles = $pedidos_detalles->fetch_object()){ 
                            echo "<img src='../".$rowpedidos_detalles->foto1."' style='max-height:35px;'>";
